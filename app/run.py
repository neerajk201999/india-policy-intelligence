...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--max-items", type=int, default=30)
    result.add_argument("--source", action="append", default=[], help="Run only an exact source name; repeat to select several sources.")
...
def main() -> int:
    ...
    pipeline = Pipeline(root=args.root, now=now, lookback_days=args.lookback_days, offline=args.offline, max_items=max(1, args.max_items), source_names=args.source)
    result = pipeline.run()
